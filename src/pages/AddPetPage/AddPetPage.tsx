...
export default function AddPetPage() {
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length < 2) { setSuggestions([]); setIsSuggestionsOpen(false); return; }
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await getNotices({ keyword: value, limit: 5 });
        setSuggestions(res.results);
        setIsSuggestionsOpen(res.results.length > 0);
      } catch { setSuggestions([]); setIsSuggestionsOpen(false); }
    }, 400);
  };
  ...
  const handleSelectSuggestion = (notice: Notice) => {
    ...
    setValue('species', notice.species.charAt(0).toUpperCase() + notice.species.slice(1));
  ...
  return (
    <main className={styles.main}>
      <div className={styles.heroWrap}>
        <img
          src="/images/add-pet@1x.png"
          srcSet="/images/add-pet@1x.png 1x, /images/add-pet@2x.png 2x"
          alt=""
      ...
      <div className={styles.formWrap}>
        ...
        <div className={styles.sexRow}>
          {[
            { value: 'female', bgClass: styles.sexBtnFemale, activeClass: styles.sexBtnFemaleActive, iconClass: styles.sexIconFemale, icon: 'icon-female' },
            { value: 'male', bgClass: styles.sexBtnMale, activeClass: styles.sexBtnMaleActive, iconClass: styles.sexIconMale, icon: 'icon-male' },
            { value: 'unknown', bgClass: styles.sexBtnUnknown, activeClass: styles.sexBtnUnknownActive, iconClass: styles.sexIconUnknown, icon: 'icon-other' },
            ...
            return (
              ...
              >
                <input type="radio" {...register('sex')} value={value} className={styles.sexInput} />
                <svg
                ...
                >
                  <use href={`/sprite/sprite.svg#${isActive ? `${icon}-white` : icon}`} />
                </svg>
              </label>
            );
          })}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
            ...
            {isSuggestionsOpen && (
              <div className={styles.suggestionsMenu}>
                {suggestions.map(notice => (
                  ...
                  >
                    {notice.imgURL && (
                      <img src={notice.imgURL} alt={notice.name} className={styles.suggestionImg} />
                    )}
                    <span className={styles.suggestionText}>
                      <span className={styles.suggestionTitle}>{notice.title}</span>
                      <span className={styles.suggestionSub}>{notice.name} · {notice.species}</span>
                    </span>
                  </button>
                ))}
              </div>
          ...
          <div className={styles.bottomRow}>
            <div className={styles.fieldWrap}>
              ...
              />
              <svg width={18} height={18} className={styles.calendarIcon} aria-hidden="true">
                <use href="/sprite/sprite.svg#icon-calendar" />
              </svg>
              {errors.birthday && (
                <span className={styles.errorMsg}>{errors.birthday.message}</span>
              )}
            </div>
            ...
            <div className={styles.selectWrapper}>
              ...
              {errors.species && (
                <span className={styles.errorMsg}>{errors.species.message}</span>
              )}
            </div>
          </div>
